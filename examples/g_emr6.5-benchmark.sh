...
jobname="tpcds-benchmark-emr-eks-3t-arm-"${testname}
...
--job-driver '{
  "sparkSubmitJobDriver": {
      ...
      "sparkSubmitParameters": "--class com.amazonaws.eks.tpcds.BenchmarkSQL --conf spark.driver.cores=4 --conf spark.driver.memory=5g --conf spark.executor.cores='${SPARK_EXCUTOR_CORE}' --conf spark.executor.memory='${SPARK_EXECUTOR_MEMORY}' --conf spark.executor.instances='${SPARK_EXECUTOR_INSTANCES}'"}}' \
--configuration-overrides '{
    "applicationConfiguration": [
      {
        "classification": "spark-defaults",
        "properties": {
          "spark.kubernetes.container.image": "'$ECR_URL'/eks-spark-benchmark:emr6.5-arm64",
          "spark.kubernetes.driver.podTemplateFile": "s3://'$S3BUCKET'/code/emr-on-eks-benchmark/examples/pod-template/g-driver-pod-template.yaml",
          "spark.kubernetes.executor.podTemplateFile": "s3://'$S3BUCKET'/code/emr-on-eks-benchmark/examples/pod-template/g-executor-pod-template.yaml",
